...
function Journal() {
    ...
    const sortPopoverId = sortOpen ? 'sort-options-popover' : undefined;
    const filterPopoverId = filterOpen ? 'filter-options-popover' : undefined;
        ...
        <>
            <div className = 'bg-[var(--tomoi-yellow-l)]'>
                ...
                <div ref={root} className='flex flex-col min-h-dvh w-full overflow-y-auto justify-start items-center'>
                    <div className='w-[90%] bg-[var(--tomoi-gray)] rounded-xl px-6 py-3 shadow-md/20 flex justify-between items-stretch items-center mt-3'>
                    ...
                    <div className='w-full flex justify-center items-start z-2 mt-3 mb-10'>
                        {
                            ...
                            <div className='relative shadow-md/20 flex flex-col gap-4 p-4 rounded-xl justify-between w-[90%] min-h-screen bg-[var(--tomoi-gray)]'>
                                    ...
                                    </div>}
                                {
                                        ...
                                        <div className='grid grid-cols-4 h-fit gap-4 w-full items-contain auto-cols-[minmax(0,3fr)]'>
                                            {
                                                entrySet.map((entry, index) => {
                                                    return  <div key={index} onClick={() => {openSelectedEntry(entry.dateCreated)}} className='relative overflow-hidden bg-[var(--tomoi-white)] h-[30vh] p-4 rounded-xl shadow-md/20 hover:shadow-md/40 flex flex-col items-start justify-end group'>
                                                                <ThreeDotsVertical width={"1.5em"} height={"1.5em"} post-date={entry.dateCreated} className='absolute right-3 top-3 rounded-full z-100 p-1 bg-transparent opacity-0 outline-white outline-6 group-hover:opacity-100 hover:outline-1 hover:outline-black group-hover:bg-[var(--tomoi-gray)] pointer-events-auto' onClick={(e) => {e.stopPropagation(); openPopover(e)}}></ThreeDotsVertical>
                                    ...
                                    :
                                    !loading &&
                                    <div className='rounded-xl h-screen w-full items-center justify-center flex flex-col gap-5 z-10'>
                                        <EmojiFrown width={'9em'} height={'9em'} className='text-[var(--tomoi-gray-d)]'></EmojiFrown>
                                        <div className='text-lg text-[var(--tomoi-gray-d)]'>No entries found.</div>
                                    </div>
                                }
            ...
            <Popover
                ...
                slotProps={{
                    paper: {
                        sx: {
                            width: sortAnchorEl?.offsetWidth,
                            backgroundColor: 'var(--tomoi-white)',
                            border: '1px dashed black',
                            borderRadius: '8px',
                            mt: 1,
